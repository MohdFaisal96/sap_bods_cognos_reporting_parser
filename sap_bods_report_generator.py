...
def drect(username, password, hostURL, arg):
    ...
    payload = '<attrs xmlns="' + login_link + '"><attr name="userName" type="string">' + username + '</attr><attr name="password" type="string">' + password + '</attr><attr name="auth" type="string" possibilities="secEnterprise,secLDAP,secWinAD,secSAPR3">secLDAP</attr></attrs>'
    ...
    print('response doc link_2:', response)
    ...
    print('response_text_2:', response_2.text)
    ...
    try:
        ...
        for index in df_universe_temp.index:
            if df_universe_temp['dataSourceType'][index] == 'unv' or df_universe_temp['dataSourceType'][index] == 'unx':
                universe_link = "http://imncbo.xglbsnet.com:6405/biprws/raylight/v1/universes/" + \
        ...
        ruleid = []
        for id in doc_list:
            # conditionlink='http://imncbo.xglbsnet.com:6405/biprws/raylight/v1/documents/1298294/reports/910/elements/926/'
...
def GetPayload(login_link, UserID, Password):
    payload = '<attrs xmlns="' + login_link + '"><attr name="userName" type="string">' + UserID + '</attr><attr name="password" type="string">' + Password + '</attr><attr name="auth" type="string" possibilities="secEnterprise,secLDAP,secWinAD,secSAPR3">secLDAP</attr></attrs>'
    return (payload)


def GetLoginToken_unv(login_link, UserID, Password):
    url = 'http://imncbo.xglbsnet.com:6405/biprws/logon/long'
...
def getJSON(UserID, Password, LoginToken, FolderID, login_link):
    doc_link = 'http://imncbo.xglbsnet.com:6405/biprws/raylight/v1/universes/' + FolderID
    headers = {
        'content-type': 'application/x-www-form-urlencoded',
        'accept': 'application/json',
...
def GetLoginToken():
    ...
    payload = GetPayload(login_link, login_cred['username'], login_cred['password'])
    headers = {
        'content-type': 'application/xml',
        'accept': 'application/xml'
    }
...
def parallel():
    ...
    result = Parallel(n_jobs=num_cores - 1)(delayed(drect)(info['username'], info['password'], info['hostURL']
                                                           , info['RepID'][i]) for i in range(0, len(info['RepID'])))
